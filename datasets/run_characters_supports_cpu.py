...
# -------------------------------------------------------
# PLAYWRIGHT SKILLS JSON FIND + scrape_skills.py
# (merged from test.py)
# -------------------------------------------------------
...
def ensure_playwright_chromium_installed():
    """
    Tries to launch Playwright Chromium. If the browser binaries are missing,
    runs `python -m playwright install chromium` and tries again.
    """
    try:
...
def find_skills_json_url_via_browser(timeout_ms: int = 60000) -> str:
    with sync_playwright() as p:
        ...
        try:
            ...
            # Fallback: catch the response instead
            try:
        ...
        if not found_url:
            raise RuntimeError(
                ...
                "Likely the site is using a different endpoint now (API) or blocking headless."
            )
        return found_url


def run_scrape_skills(json_url: str):
...
def run_skills_step_after_npm_check():
    """
    Runs the test.py steps AFTER npm check:
      - ensure_playwright_chromium_installed()
      - find_skills_json_url_via_browser()
      - run scrape_skills.py to update in_game/skills.json
    """
    ensure_playwright_chromium_installed()
...
def main():
    ...
    # 2) AFTER npm check: run Playwright skills-json -> scrape_skills.py
    ...
    # -------------------------------------------------------
    # NEW: Before the FIRST call to scrape_events.py, delete:
    #   web\public\events\support
    #   web\public\events\trainee
    # -------------------------------------------------------
    project_root = SCRIPT_DIR.parent
    support_events_dir = project_root / "web" / "public" / "events" / "support"
    trainee_events_dir = project_root / "web" / "public" / "events" / "trainee"

    for p in (support_events_dir, trainee_events_dir):
        if p.exists():
            print(f"[INFO] Removing existing directory tree: {p}")
            try:
                shutil.rmtree(p)
            except Exception as e:
    ...
    # Scrape ONLY missing (this is where scrape_events.py is first called)
